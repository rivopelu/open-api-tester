...
export function McpConnectionPanel() {
  ...
  const revoke = async () => {
    if (!window.confirm('Revoke the MCP token? Connected clients will lose access immediately.')) return
    setRevoking(true)
    try {
    ...
    <section className="mb-8 border border-border bg-surface">
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <Typography variant="heading-sm" as="span">Connect your AI client</Typography>
          <Typography variant="body-sm" tone="muted" as="span" className="mt-1 block">
            Give your LLM read-only access to projects and endpoint specifications through MCP.
          </Typography>
        </span>
      ...
      {expanded && (
        <div className="grid border-t border-border lg:grid-cols-[minmax(250px,0.7fr)_minmax(0,1.3fr)]">
          <div className="border-b border-border p-5 lg:border-b-0 lg:border-r">
            <Typography variant="label" as="h3" className="mb-3">Connection credentials</Typography>
            <label className="mb-2 block text-xs font-semibold text-text-secondary">MCP endpoint</label>
            <div className="mb-5 flex h-9 border border-border bg-base">
              <code className="min-w-0 flex-1 truncate px-3 py-2 text-[11px] text-text-secondary">{endpoint}</code>
              <button type="button" onClick={() => void copy('endpoint', endpoint)} className="border-l border-border px-3 text-text-muted hover:text-primary" aria-label="Copy MCP endpoint">
                {copied === 'endpoint' ? <Check className="h-3.5 w-3.5 text-success" /> : <Copy className="h-3.5 w-3.5" />}
              </button>
            </div>
            ...
            <div className="mb-3 flex h-9 border border-border bg-base">
              ...
              {token && (
                <button type="button" onClick={() => void copy('token', token)} className="border-l border-border px-3 text-text-muted hover:text-primary" aria-label="Copy MCP token">
          ...
          <div className="min-w-0 p-5">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
              <Typography variant="label" as="h3">Client setup</Typography>
              <div className="flex border border-border bg-base" role="tablist" aria-label="MCP clients">
            ...
            <div className="relative min-h-[150px] border border-border bg-base p-4 pr-12">
              ...
              <button type="button" onClick={() => void copy('config', config)} className="absolute right-3 top-3 p-1.5 text-text-muted hover:text-primary" aria-label="Copy MCP configuration">
                {copied === 'config' ? <Check className="h-4 w-4 text-success" /> : <Copy className="h-4 w-4" />}
              </button>
            </div>
            <div className="mt-4 grid gap-2 text-xs text-text-secondary sm:grid-cols-3">
              <span><strong className="mr-2 font-mono text-primary">1</strong>Generate token</span>
              <span><strong className="mr-2 font-mono text-primary">2</strong>Copy configuration</span>
              <span><strong className="mr-2 font-mono text-primary">3</strong>Restart your client</span>
            </div>
          </div>
        </div>
      )}
