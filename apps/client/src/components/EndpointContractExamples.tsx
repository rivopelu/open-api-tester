...
function sameSelection(left: SelectedExample | null, right: SelectedExample | null): boolean {
  if (!left || !right) return left === right
  if (left.scope !== right.scope || left.id !== right.id) return false
  return left.scope === 'request' || right.scope === 'request' || left.responseId === right.responseId
}
...
function findSelectionById(
  ...
  if (requestExamples.some((example) => example.id === exampleId)) return { scope: 'request', id: exampleId }

  const response = responses.find((item) => item.examples?.some((example) => example.id === exampleId))
  return response ? { scope: 'response', responseId: response.id, id: exampleId } : null
}

function firstSelection(requestExamples: EndpointExample[], responses: ResponseDefinition[]): SelectedExample | null {
  if (requestExamples[0]) return { scope: 'request', id: requestExamples[0].id }
  for (const response of responses) {
    if (response.examples?.[0]) {
      return { scope: 'response', responseId: response.id, id: response.examples[0].id }
    }
  }
  return null
}
...
function selectionExists(
  ...
  if (selection.scope === 'request') return requestExamples.some((example) => example.id === selection.id)
  return responses.some((response) => (
    response.id === selection.responseId && response.examples?.some((example) => example.id === selection.id)
  ))
}
...
function validateGroup(
  ...
  for (const example of examples) {
    ...
    if (!name) {
      return { type: 'validation', selection, target: 'name', message: `${label} example name cannot be empty.` }
    }
    if (names.has(name)) {
      return { type: 'validation', selection, target: 'name', message: `${label} contains more than one example named “${name}”. Use a unique name.` }
    }
    names.add(name)
    ...
    try {
      ...
      return { type: 'validation', selection, target: 'payload', message: `${name} contains invalid JSON. Fix the payload before saving.` }
    }
  }
...
function validateDrafts(requestExamples: EndpointExample[], responses: ResponseDefinition[]): Feedback | null {
  const requestIssue = validateGroup(requestExamples, 'Request', (id) => ({ scope: 'request', id }))
...
function statusTone(statusCode: string): string {
  if (/^2\d\d$/.test(statusCode)) return 'border-success/40 bg-success/10 text-success'
  if (/^[45]\d\d$/.test(statusCode)) return 'border-danger/40 bg-danger/10 text-danger'
  if (/^3\d\d$/.test(statusCode)) return 'border-warning/40 bg-warning/10 text-warning'
...
export function EndpointContractExamples({ endpoint, initialExampleId, onSelectExample, onSave, onDirtyChange }: Props) {
  ...
  ), [persistedRequestSignature, persistedResponseSignature, requestExamples, responses])
  ...
  useEffect(() => {
    ...
    setFeedback(null)
    setSelected((current) => {
      if (selectionExists(current, nextRequestExamples, nextResponses)) return current
  ...
  useEffect(() => {
    const requested = findSelectionById(initialExampleId, requestExamples, responses)
    if (requested) setSelected((current) => sameSelection(current, requested) ? current : requested)
  ...
  const activeResponse = selected?.scope === 'response'
  ...
  const active = selected?.scope === 'request'
    ...
    : selected?.scope === 'response'
      ? activeResponse?.examples?.find((example) => example.id === selected.id)
      : undefined
  const activeHasFeedback = Boolean(feedback?.selection && selected && sameSelection(feedback.selection, selected))
  const activeNameHasError = activeHasFeedback && feedback?.target === 'name'
  const activePayloadHasError = activeHasFeedback && feedback?.target === 'payload'
  const totalExamples = requestExamples.length + responses.reduce((total, response) => total + (response.examples?.length ?? 0), 0)
  ...
  const removeActive = () => {
    ...
    if (selected.scope === 'request') {
      const nextRequestExamples = requestExamples.filter((item) => item.id !== selected.id)
    ...
    const nextResponses = responses.map((response) => response.id === selected.responseId
      ? { ...response, examples: (response.examples ?? []).filter((item) => item.id !== selected.id) }
      : response)
    const sameGroup = nextResponses.find((response) => response.id === selected.responseId)?.examples?.[0]
    const nextSelection = sameGroup
      ? { scope: 'response' as const, responseId: selected.responseId, id: sameGroup.id }
  ...
  const save = useCallback(async () => {
    const hasChanges = JSON.stringify(requestExamples) !== JSON.stringify(endpoint.requestBody?.examples ?? [])
    ...
    try {
      ...
      setFeedback({
        type: 'save',
        message: error instanceof Error ? error.message : 'Contract examples could not be saved. Try again.',
      })
    } finally {
      setSaving(false)
    }
  }, [endpoint.requestBody, endpoint.responses, onSave, onSelectExample, requestExamples, responses, saving])
      ...
      <aside className="flex max-h-72 min-h-0 flex-col border-b border-border bg-overlay/50 lg:max-h-none lg:border-b-0 lg:border-r">
        <div className="flex h-12 shrink-0 items-center gap-2 border-b border-border px-3">
          <div className="min-w-0 flex-1">
            <Typography variant="label" as="h3" className="truncate">Contract examples</Typography>
            <Typography variant="caption" tone="muted" as="p">{totalExamples} payload{totalExamples === 1 ? '' : 's'}</Typography>
          </div>
          {dirty && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-warning" title="Unsaved changes" aria-label="Unsaved changes" />}
          <Button variant="ghost" size="sm" iconOnly onClick={() => void save()} loading={saving} disabled={!dirty} aria-label="Save contract examples">
        ...
        <div className="scroll-y min-h-0 flex-1 py-2">
          <section aria-labelledby="request-examples-heading">
            <div className="flex h-8 items-center justify-between px-3">
              <span id="request-examples-heading" className="text-[10px] font-bold uppercase tracking-[0.12em] text-text-muted">Request</span>
              <button type="button" onClick={addRequest} className="p-1 text-text-muted transition-colors hover:bg-surface hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label="Add request example">
                <Plus className="h-3.5 w-3.5" />
              </button>
            </div>
            {requestExamples.length === 0 ? (
              <button type="button" onClick={addRequest} className="mx-2 flex w-[calc(100%-1rem)] items-center gap-2 border border-dashed border-border px-2.5 py-2 text-left text-[10px] text-text-muted transition-colors hover:border-primary/50 hover:bg-primary/5 hover:text-text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
              ...
              <button
                key={example.id}
                type="button"
                onClick={() => selectExample({ scope: 'request', id: example.id })}
                aria-pressed={selected?.scope === 'request' && selected.id === example.id}
                className={cn(
              ...
              >
                <FileJson2 className="h-3.5 w-3.5 shrink-0" /><span className="truncate">{example.name || 'Unnamed example'}</span>
              </button>
            ))}
          </section>

          <div className="mx-3 my-2 border-t border-border" />
          <div className="px-3 pb-1 text-[10px] font-bold uppercase tracking-[0.12em] text-text-muted">Responses</div>
          {responses.length === 0 ? (
            <p className="px-3 py-2 text-[10px] leading-4 text-text-muted">Add a response definition before creating response examples.</p>
          ) : responses.map((response) => {
            const examples = response.examples ?? []
            return (
              <section key={response.id} className="mb-2" aria-labelledby={`response-${response.id}-heading`}>
                <div className="flex min-h-8 items-center gap-2 px-3">
                  <span id={`response-${response.id}-heading`} className={cn('border px-1.5 py-0.5 font-mono text-[10px] font-bold', statusTone(response.statusCode))}>{response.statusCode}</span>
                  <span className="min-w-0 flex-1 truncate text-[10px] text-text-muted">{response.description || 'Response'}</span>
                  <button type="button" onClick={() => addResponse(response.id)} className="p-1 text-text-muted transition-colors hover:bg-surface hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary" aria-label={`Add ${response.statusCode} response example`}>
                    <Plus className="h-3.5 w-3.5" />
                  </button>
                </div>
                {examples.length === 0 ? (
                  <button type="button" onClick={() => addResponse(response.id)} className="mx-3 block text-[10px] text-text-muted transition-colors hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">No examples · Add one</button>
                ) : examples.map((example) => (
                  <button
                    key={example.id}
                    type="button"
                    onClick={() => selectExample({ scope: 'response', responseId: response.id, id: example.id })}
                    aria-pressed={selected?.scope === 'response' && selected.id === example.id}
                    className={cn(
                      ...
                      selected?.scope === 'response' && selected.id === example.id
                        ? 'border-primary bg-primary/10 text-primary'
                        : 'border-transparent text-text-secondary hover:bg-surface hover:text-text-primary',
                    )}
                  >
      ...
      <section className="flex min-h-107.5 min-w-0 flex-col bg-base">
        {!active ? (
          <div className="flex min-h-107.5 flex-1 flex-col items-center justify-center px-6 py-10 text-center">
            ...
            <Typography variant="label" as="p" className="mb-1">No contract example selected</Typography>
            <Typography variant="body-sm" tone="muted" className="mb-4 max-w-md">Create a named request or response payload for documentation, testing, and generated OpenAPI output.</Typography>
            <div className="flex flex-wrap justify-center gap-2">
              <Button variant="primary" size="sm" onClick={addRequest}><Plus className="h-3.5 w-3.5" />Add request example</Button>
              {responses[0] && <Button variant="outline" size="sm" onClick={() => addResponse(responses[0].id)}><Plus className="h-3.5 w-3.5" />Add {responses[0].statusCode} response</Button>}
            </div>
          </div>
        ) : (
          <>
            <div className="flex min-h-12 shrink-0 items-center gap-3 border-b border-border bg-surface px-4">
              ...
              <div className="min-w-0 flex-1">
                <Typography variant="label" as="h3" className="truncate">{active.name || 'Unnamed example'}</Typography>
              ...
              {selected?.scope === 'response' && activeResponse && (
                <span className={cn('border px-2 py-1 font-mono text-[10px] font-bold', statusTone(activeResponse.statusCode))}>{activeResponse.statusCode}</span>
              )}
              <Button variant="danger" size="sm" onClick={removeActive} aria-label={`Delete ${active.name || 'example'}`}>
            ...
            <div className="grid shrink-0 border-b border-border bg-surface md:grid-cols-[minmax(160px,0.8fr)_minmax(220px,1.2fr)]">
              <label className="min-w-0 border-b border-border p-3 md:border-b-0 md:border-r">
                ...
                <input
                  value={active.name}
                  onChange={(event) => updateActive({ name: event.target.value.replace(/\s+/g, '_') })}
            ...
            <CodeEditor
              ...
              label={selected?.scope === 'request' ? 'Request payload' : `${activeResponse?.statusCode ?? ''} response payload`}
              className={cn('min-h-65 flex-1', activePayloadHasError && 'border border-danger')}
            />
            ...
            <div className="flex min-h-14 shrink-0 flex-col gap-3 border-t border-border bg-surface px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              <div role={feedback ? 'alert' : 'status'} className={cn('flex min-w-0 items-start gap-2 text-[11px]', feedback ? 'text-danger' : saved ? 'text-success' : dirty ? 'text-warning' : 'text-text-muted')}>
                {feedback ? <AlertCircle className="mt-0.5 h-3.5 w-3.5 shrink-0" /> : saved ? <Check className="mt-0.5 h-3.5 w-3.5 shrink-0" /> : null}
